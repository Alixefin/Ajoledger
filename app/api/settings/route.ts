...
function checkServerRateLimit(ip: string, limit = 10, windowMs = 60000): boolean {
  const now = Date.now()
...
export async function PUT(request: NextRequest) {
  try {
    ...
    const validation = validateForm(settingsSchema.partial(), updateData)
